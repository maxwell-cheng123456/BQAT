...
def complete_adjacency_matrix(adj_list, bond_types):
    ...
    for i, neighbors in enumerate(adj_list):
        for j in neighbors:
            ...
            if key in bond_types:
                matrix[i][j] = bond_types[key]
    return matrix
def get_matrix(smiles):
    graph = SmilesGraph(smiles)
    adjacency_matrix, bond_types, bond_attributes = \
        graph.extract_graph_structure(smiles)
    matrix = complete_adjacency_matrix(adjacency_matrix, bond_types)
    return np.array(matrix)
def get_energy_matrix(smiles,vocab=None):
    tokens_list = smi_tokenizer(smiles)
    if vocab==None:

        vocab = torch.load('../vocab/vocab_with_error.pt')
    Atom_List = [locate_atom.find_atom(str(vocab[i])) for i in tokens_list]
    matrix = get_matrix(smiles)
    bond_energy_matrix = np.array(matrix)
    bond_energy_matrix=bond_energy_matrix.astype(np.float64)
    for i in range(len(matrix)):
        for j in range(len(matrix)):
            if matrix[i][j] != 0:
                e = locate_atom.get_bond_energy(Atom_List[i], Atom_List[j], matrix[i][j])
                if e == 'nan':
                    e = np.nan
                bond_energy_matrix[i][j] = e
    bond_energy_matrix = np.log(bond_energy_matrix/100+1)*1
    return bond_energy_matrix
def get_bondlength_matrix(smiles,vocab=None):
    tokens_list = smi_tokenizer(smiles)
    if vocab==None:
        vocab = torch.load('../vocab/vocab_with_error.pt')
    Atom_List = [locate_atom.find_atom(str(vocab[i])) for i in tokens_list]
    matrix = get_matrix(smiles)
    bond_length_matrix = np.array(matrix)
    bond_length_matrix=bond_length_matrix.astype(np.float64)
    for i in range(len(matrix)):
        for j in range(len(matrix)):
            if matrix[i][j] != 0:
                e = locate_atom.get_bond_length(Atom_List[i], Atom_List[j], matrix[i][j])
                bond_length_matrix[i][j] = e
            else:
                bond_length_matrix[i][j] = np.inf
    bond_length_matrix = 1/(bond_length_matrix/1000)
    bond_length_matrix = np.log(bond_length_matrix + 1) * 1
    return bond_length_matrix
if __name__=='__main__':
    smiles='C=C.c1ccccc1.CC.C#C'
    tokens_list=smi_tokenizer(smiles)
    print(tokens_list)
    import torch
    import locate_atom
    ...
    for i in range(len(matrix)):
        for j in range(len(matrix)):
            if matrix[i][j]!=0:
                ...
                bond_energy_matrix[i][j]=e
    print(bond_energy_matrix)
    bond_energy_matrix=get_energy_matrix(smiles)
    print(bond_energy_matrix)

    smiles='C=CC=O'
    bm=get_matrix(smiles)
    print(bm)
    be=get_bondlength_matrix(smiles)
    print(be)
    print(np.round(be,2))
